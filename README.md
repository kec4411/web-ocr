...
| 実行環境 | Docker Compose |
| テスト | pytest (6件) / Vitest + Testing Library (5件) |
...
カバレッジを追わず、実際に発生したバグを守る11件に絞りました。最も価値が高いのは `test_japanese_langpack_installed` で、`{'jpn','eng'} <= set(pytesseract.get_languages())` を検証するだけの高速なテストですが、上記の言語バグを直接守ります。
...
結果: `.git` 88MB → **720KB**、`git clone` 608MB → **1.0MB**。
